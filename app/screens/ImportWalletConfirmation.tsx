...
const ImportWalletVerify: React.FC<ImportWalletVerifyProps> = ({ navigation }) => {
  ...
  const initAgent = async (seed: string): Promise<void> => {
    ...
    try {
      ...
      const walletFilePath = `${restoreDirectoryPath}/Medera_WALLET_RESTORE/Medera_WALLET.wallet`

      await unzip(selectedFilePath, restoreDirectoryPath + '/Medera_WALLET_RESTORE')
      ...
      await RNFS.unlink(restoreDirectoryPath + '/Medera_WALLET_RESTORE')
